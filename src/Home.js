export default function Home() {
  return (
    <div>
      <div className="p-6 h-screen bg-green-900 grid md:grid-cols-3 grid-cols-4 grid-rows-3">
        <div className="font-body p-5 row-start-2 row-span-1 md:col-start-2 md:col-span-1 
        col-start-1 animate-bounce-3 col-span-4 grid place-items-center text-center 
        bg-gray-200 text-black rounded-lg border-2 border-black shadow-lg">
          <p className="">Hi, I'm Patrick 👋</p>
          <p>I'm a full-stack developer, and I love learning new things.</p>
        </div>
      </div>
      <div className="h-screen bg-green-700">

      </div>
      </div>
  );
}